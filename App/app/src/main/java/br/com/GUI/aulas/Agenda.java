...
import br.com.CustomListView.CustomAdapterAgendamentos;
import br.com.CustomListView.RowItemAgendamentos;
...
public class Agenda extends Fragment {
	...
	public void onActivityCreated (Bundle savedInstanceState){
		...
		opcoesFiltro.add(getString(R.string.label_hoje));
		opcoesFiltro.add(getString(R.string.label_neste_mes));
		opcoesFiltro.add(getString(R.string.label_todas));
	...
	public void atualizarAgenda(String filtro){
		...
		if (pref.getString("tipo", null).equals("personal")){
			...
			for(Aula a : al){
				if(spnFiltrarAulas.getSelectedItem().toString().equals(getString(R.string.label_hoje))){
				...
				}else if(spnFiltrarAulas.getSelectedItem().toString().equals(getString(R.string.label_neste_mes))){
					if( a.getMesAula() == c.get(Calendar.MONTH)
							&& a.getAnoAula() == c.get(Calendar.YEAR)){
						exibir.add(a);
						
					}
				}else if(spnFiltrarAulas.getSelectedItem().toString().equals(getString(R.string.label_todas))){
					exibir.add(a);
				}
			}
	...
	public void atualizarInterface(ArrayList<Aula> exibir){
		...
		for (Aula l : exibir) {
			String dataAula = l.getMesAula() + "/" + l.getDiaAula() + "/" + l.getAnoAula();
			String anoAula = l.getHoraAula() + ":" + l.getMinutoAula();
			
			String confirmacao;
			if(l.getConfirmacaoAulaAluno() == 1 && l.getConfirmacaoAulaPersonal() == 1){
				confirmacao = getString(R.string.label_confirmada);
			}else if(l.getConfirmacaoAulaAluno() == 0 && l.getConfirmacaoAulaPersonal() == 0 ){
				confirmacao = getString(R.string.label_cancelada);
			}else{
				confirmacao = getString(R.string.label_aguardando_confirmacao);
			}
